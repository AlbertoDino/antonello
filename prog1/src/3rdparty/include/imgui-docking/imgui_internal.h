...
static inline float  ImLinearSweep(float current, float target, float defaultSpeed)    { if (current < target) return ImMin(current + defaultSpeed, target); if (current > target) return ImMax(current - defaultSpeed, target); return current; }
